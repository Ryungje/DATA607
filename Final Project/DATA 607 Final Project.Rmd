...
Preliminary data cleaning is needed for both data sets. To begin, tidying and cleaning will be performed on each dataset seperately. Afterwards, further cleaning will be done to make each data set appear more like each other.
...
For the most part, the ***nyc_api*** dataset is in good order, but there is still some minor cleaning needed. For starters, we will remove the year column. 
...
### Personal Notes
if you guys don't want to get rid of all the missing values, we can take that out
```{r}
library(ggmap)
library(ggplot2)

# e.g. center at approx NYC coordinates
nyc_center <- c(lon = -74.0060, lat = 40.7128)

# get a map (using Stamen to avoid API-key issues)
nyc_map <- get_map(location = nyc_center, zoom = 11,
                   source = "stamen", maptype = "toner-lite")

# plot the map and add points from your data frame
ggmap(nyc_map) +
  geom_point(data = nyc_api,
             aes(x = longitude, y = latitude),
             colour = "red",
             alpha = 0.6,
             size = 2) +
  labs(title = "NYC data points",
       x = "Longitude",
       y = "Latitude") +
  theme_minimal()
