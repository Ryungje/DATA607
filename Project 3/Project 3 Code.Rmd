...
***As of now, the relevant datasets are stored in a github repo and accessed/imported as raw github file.*** Note that the postings.csv from the LinkedIn Data takes considerable time to import using this method.
```{r}
...
These two graphs are simple bar graphs which display the most frequently appearing skills from each data set.  
...
DS_job_skills %>%
  ...
  coord_flip() + 
  labs(x= "Data Science Job Skills", y = "Count", title = "Data Science Skills in Job Postings")
```
...
LI_job_skills %>%
  ...
  coord_flip() +
  labs(x= "LinkedIn Job Skills", y = "Count", title = "LinkedIn Skills in Job Postings")
```

```{r}
temp <- DS_job_postings %>%
