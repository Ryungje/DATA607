...
LI_job_postings <- read.csv("LinkedIn Job Postings/postings.csv")
...
***DS_job_skills*** contains the respective job skills for each listing in ***job_posting***. They are relate to each other through *job_link*. The skills are presented in a single string, which will require further work to parse each individual skill.
...
Work will first be done to tidy ***DS_job_postings***. We will be keeping only select columns in ***DS_job_postings*** and merging in information from ***DS_job_skills***. We will also split the skill strings and, thus, elongate ***DS_job_postings***. 
...
DS_job_skills %>%
  ...
  coord_flip() + 
  labs(x= "Data Science Job Skills", 
       y = "Count", 
       title = "Skills in Data Science Job Postings")
```
...
LI_job_skills %>%
  ...
  coord_flip() +
  labs(x= "LinkedIn Job Skills", 
       y = "Count", 
       title = "Skills in LinkedIn Job Postings",
       caption = "Includes all job skills, even the ones irrelevant to Data Science") +
  # Caption customizing
  theme(
...
In this next graph we filter job skills from ***DS_job_postings*** that specifically mention "Data" and present the top ten. This graph will reappear later.
...
<br>
Some additional tidying to ***DS_job_postings***. Changing all instances that contains "Data Analyst" or anything similar, to say "Data Analysis." After all, these all mean the same thing: having data analysis skills.  
...
DS_job_postings %>%
  ...
  labs(x= "Data Job Skills", 
       y = "Count", 
       title = "Data Skills in Data Science Job Postings",
       caption = "Data Analysis counts increased nearly by a thousand") +
  # Caption customizing
  theme(
    plot.caption = element_text(
      hjust = 0.5,        # 0 = left, 0.5 = center, 1 = right
